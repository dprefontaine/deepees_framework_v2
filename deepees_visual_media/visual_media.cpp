...
void visual_media::rgb_to_hsl(){
    float r = color_and_transparency.r/255.0;
    float g = color_and_transparency.g/255.0;
    float b = color_and_transparency.b/255.0;
    ...
    //SETTING LUMINOSITY
    set_luminosity((max_col+min_col)/2*240);
    ...
    if (max_col == min_col){
        ...
        if (luminosity < 120)
            set_saturation(((max_col-min_col)/(max_col+min_col))*240.0);
        else
            set_saturation(((max_col-min_col)/(2-max_col-min_col))*240.0);
    }

    //SETTING HUE
    //FIND WHICH ONE IS MAX
    if (max_col == r)
        set_hue(((g-b)/(max_col-min_col))*239*2/3);
    else if (max_col == g)
        set_hue((2+((b-r)/(max_col-min_col)))*239*2/3);
    else if (max_col == b)
        set_hue((4+(g-r)/(max_col-min_col))*239*2/3);
}

void visual_media::hsl_to_rgb(){
    float temp_calc_1, temp_calc_2, temp_r, temp_g, temp_b;

    if (luminosity < 120)
        temp_calc_1 = (luminosity*(240+(saturation)))/240.0;
    else
        temp_calc_1 = ((luminosity)+(saturation)-(luminosity*saturation))/240.0;

    temp_calc_2 = (2*luminosity/240)-temp_calc_1;

    temp_r = hue/239.0+0.333;
    temp_g = hue/239.0;
    temp_b = hue/239.0-0.333;

    if (temp_r*6 < 1){
        set_red((temp_calc_2 + (temp_calc_1 - temp_calc_2)*6 + temp_r)*255);
    } else if (temp_r*2 < 1){
        set_red(temp_calc_1*255);
    } else if (temp_r*3 < 1){
        set_red((temp_calc_2 + (temp_calc_1 - temp_calc_2)*(0.667 - temp_r)*6)*255);
    } else {
        set_red(temp_calc_2*255);
    }

    if (temp_g*6 < 1){
        set_green((temp_calc_2 + (temp_calc_1 - temp_calc_2)*6 + temp_g)*255);
    } else if (temp_g*2 < 1){
        set_green(temp_calc_1*255);
    } else if (temp_g*3 < 1){
        set_green((temp_calc_2 + (temp_calc_1 - temp_calc_2)*(0.667 - temp_g)*6)*255);
    } else {
        set_green(temp_calc_2*255);
    }

    if (temp_b*6 < 1){
        set_blue((temp_calc_2 + (temp_calc_1 - temp_calc_2)*6 + temp_b)*255);
    } else if (temp_b*2 < 1){
        set_blue(temp_calc_1*255);
    } else if (temp_b*3 < 1){
        set_blue((temp_calc_2 + (temp_calc_1 - temp_calc_2)*(0.667 - temp_b)*6)*255);
    } else {
        set_blue(temp_calc_2*255);
    }
}
